...
async function orderSuccess(req, res) {
    try {
        ...
        return res.status(200).json({ message: "payment sent successfully ", result: data });
    } catch (error) {
        return res.status(400).json({ message: "something went wrong", result: error.message })
    }
}
...
async function orderCancel(req, res) {
    try {
        let data = await order.orderCancel(req,res)
        return res.status(200).json({ message: "payment cancel successfully ", result: data });
    } catch (error) {
        return res.status(400).json({ message: "something went wrong", result: error.message })
    }
}
